...
//模型变换
Matrix Rasterizer::get_model_matrix(char c,float rotation)
{
...
//摄像机变换
Matrix Rasterizer::get_camera_matrix(Vec3f camera, Vec3f center, Vec3f up)
{
        //先计算z，根据z和up算出x，再算出y
...
//投影变换
Matrix Rasterizer::get_projection_matrix(Vec3f camera, Vec3f center)
{
    Matrix mat = Matrix::identity(4);
...
//视角变换
Matrix Rasterizer::get_viewport_matrix(int width, int height, int depth)
{
    //创建一个4x4单位矩阵mat
    Matrix mat = Matrix::identity(4);
    //第四列表示平移信息
...
void Rasterizer::triangle_line(Vec3f* pts, Vec2i *uv, Model *model, TGAImage &image ,float intensity,int *zbuffer)
{
    ...
    if(t0.y == t1.y && t0.y == t2.y)
        return;
    //将三个点根据y的大小排序，使得t2.y>t1.y>t0.y, uv,intensity坐标也进行对应排序
...
void Rasterizer::triangle_box(Vec3f *pts, Vec2i *uv, Model *model, TGAImage &image, float intensity, int *zbuffer)
{
