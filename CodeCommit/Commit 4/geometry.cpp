...
//3d->4d
//添加一个1表示坐标
Matrix::Matrix(Vec3f v) : m(std::vector<std::vector<float> >(4, std::vector<float>(1, 1.0f))), rows(4), cols(1) {
    m[0][0] = v.x;
    m[1][0] = v.y;
    m[2][0] = v.z;
}
...
template <typename T> Vec4<T> operator*(Vec4<T>& vec, Matrix& mat)
{
    Vec4f res;
    if(mat.ncols() != 4 || mat.nrows() != 4)
    {
        std::cout<<"该矩阵与Vec4f无法相乘"<<std::endl;
        return res;
    }
